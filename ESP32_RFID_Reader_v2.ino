/*
 * โปรเจกต์: ระบบเช็คชื่อด้วย RFID + Node.js Server
 * ส่งข้อมูลไปยัง Node.js Server และบันทึกในฐานข้อมูล SQLite
 * วันที่: 5 มีนาคม 2026
 * 
 * Hardware Required:
 * - ESP32 Development Board
 * - RFID-RC522 Module
 * - Buzzer (Passive Buzzer ที่รองรับ tone())
 * - RGB LED (3x LED: Red, Green, Blue)
 * 
 * การต่อสาย RFID-RC522 กับ ESP32:
 * ----------------------------------------
 * RFID RC522    |    ESP32
 * ----------------------------------------
 * SDA (SS)      |    GPIO 5
...
const char* SERVER_URL = "http://192.168.0.196:3000/api/attendance/scan";  // ✅ IP ของคอมพิวเตอร์
...
void stateWait()  { setRGB(0, 0, 1); }  // 🔵 น้ำเงิน - รอการแตะ
void stateNew()   { setRGB(0, 1, 0); }  // 🟢 เขียว - บันทึกใหม่
void stateDup()   { setRGB(1, 1, 0); }  // 🟡 เหลือง - แตะซ้ำ
void stateFail()  { setRGB(1, 0, 0); }  // 🔴 แดง - ล้มเหลว
...
SendResult postAttendance(const String& rfidUID) {
  ...
  // ตรวจสอบการ Duplicate (เช็คชื่อแล้ว)
  if (response.indexOf("เช็คชื่อแล้ว") >= 0 || response.indexOf("error") >= 0) {
    if (response.indexOf("เช็คชื่อแล้ว") >= 0) {
