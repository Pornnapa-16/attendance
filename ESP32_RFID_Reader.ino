...
// ==================== การตั้งค่า Server ====================
// ใช้โดเมน Render เพื่อให้ ESP32 เรียก API ได้แม้ไม่ได้รัน server local
// หากต้องการใช้เครื่อง local ให้เปลี่ยนเป็น http://<IPv4-เครื่องคอม>:3000/api/...
const char* SERVER_URL = "https://attendance-gmdn.onrender.com/api/attendance/scan";
const char* ACTIVE_COURSE_URL = "https://attendance-gmdn.onrender.com/api/public/active-course";
...
void syncActiveCourse() {
  ...
  bool isHttps = activeCourseUrl.startsWith("https://");

  if (isHttps) {
    secureWifiClient.setInsecure();  // ยอมรับ TLS certificate เพื่อความง่ายในการใช้งาน
    if (!http.begin(secureWifiClient, activeCourseUrl)) {
      Serial.println("⚠️ ดึงวิชาปัจจุบันไม่สำเร็จ (begin fail)");
      return;
    }
  } else {
    if (!http.begin(wifiClient, activeCourseUrl)) {
      Serial.println("⚠️ ดึงวิชาปัจจุบันไม่สำเร็จ (begin fail)");
      return;
    }
  }
